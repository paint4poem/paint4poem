...
def words_loss(img_features, words_emb, labels,
    ...
    if class_ids is not None:
        masks = np.concatenate(masks, 0)
        # masks: batch_size x batch_size
        # masks = torch.ByteTensor(masks) OWN CHANGE
        masks = torch.BoolTensor(masks)
        if cfg.CUDA:
            masks = masks.cuda()
...
def discriminator_loss(netD, real_imgs, fake_imgs, conditions,
    ...
    # print(cond_fake_logits, fake_labels)
    #
    ...
    if netD.UNCOND_DNET is not None:
        ...
        # print('UNCOND_DNET', real_logits, real_labels)
        # print(fake_logits, fake_labels)
...
def generator_loss(netsD, image_encoder, fake_imgs, real_labels,
    ...
    for i in range(numDs):
        ...
        G_errors["G{}_error".format(i)] = g_loss.item()
    ...
    return errG_total, G_errors, s_loss.item(), w_loss.item()

def generator_loss_val(netsD, image_encoder, fake_imgs, real_labels,
                   words_embs, sent_emb, match_labels,
                   cap_lens, class_ids):
    G_errors = {}   
    numDs = len(netsD)
    ...
    errG_total = 0
    for i in range(numDs):
        features = netsD[i](fake_imgs[i].detach())
        cond_logits = netsD[i].COND_DNET(features, sent_emb)
        cond_errG = nn.BCELoss()(cond_logits, real_labels)
        if netsD[i].UNCOND_DNET is  not None:
            logits = netsD[i].UNCOND_DNET(features)
        ...
        if i == (numDs - 1):
            # words_features: batch_size x nef x 17 x 17
            # sent_code: batch_size x nef
            region_features, cnn_code = image_encoder(fake_imgs[i].detach())
            w_loss0, w_loss1, _ = words_loss(region_features, words_embs,
                                             match_labels, cap_lens,
                                             class_ids, batch_size)
            w_loss = (w_loss0 + w_loss1) * \
...
def cycle_generator_loss(netsD, image_encoder, fake_imgs, real_labels, captions,
    ...
    for i in range(numDs):
        ...
        if netsD[i].UNCOND_DNET is  not None:
            logits = netsD[i].UNCOND_DNET(features)
            errG = nn.BCELoss()(logits, real_labels)
            g_loss = cfg.LOSS.G_w*DG_w_change(epoch)*errG + cond_errG
        else:
            g_loss = cond_errG
        errG_total += g_loss
        # err_img = errG_total.item()
    ...
    return errG_total, G_errors, s_loss.item(), w_loss.item(), t_loss.item()

def cycle_generator_loss_val(netsD, image_encoder, fake_imgs, real_labels, captions,
                   words_embs, sent_emb, match_labels,
                   cap_lens, class_ids):
    G_errors = {}
    numDs = len(netsD)
    ...
    for i in range(numDs):
        ...
        if i == (numDs - 1):
            # words_features: batch_size x nef x 17 x 17
            # sent_code: batch_size x nef
            region_features, cnn_code, word_logits = image_encoder(fake_imgs[i].detach(), captions)
            w_loss0, w_loss1, _ = words_loss(region_features, words_embs,
                                             match_labels, cap_lens,
                                             class_ids, batch_size)
            w_loss = (w_loss0 + w_loss1) * \
