...
class RNN_ENCODER(nn.Module):
    def __init__(self, ntoken, ninput=300, drop_prob=0.5,
                 nhidden=128, nlayers=1, bidirectional=True, model='../models/pytorch/'):
...
#     def define_module(self):
#         print('Load pretrained BERT model from ', self.model)
#         self.encoder = BertModel.from_pretrained(self.model)
#         for param in self.encoder.parameters():
#             param.requires_grad = False
#         self.bert_linear = nn.Linear(768, self.ninput)
#         print(self.bert_linear)
#         self.drop = nn.Dropout(self.drop_prob)
#         if self.rnn_type == 'LSTM':
#             # dropout: If non-zero, introduces a dropout layer on
#             # the outputs of each RNN layer except the last layer
...
#         print(self.bert_linear.weight.data.uniform_(-initrange, initrange).size())
#         # Do not need to initialize RNN parameters, which have been initialized
#         # http://pytorch.org/docs/master/_modules/torch/nn/modules/rnn.html#LSTM
#         # self.decoder.weight.data.uniform_(-initrange, initrange)
#         # self.decoder.bias.data.fill_(0)
...
class CNN_ENCODER_attn(nn.Module):
    def __init__(self, nef):
        super(CNN_ENCODER_attn, self).__init__()
        if cfg.TRAIN.FLAG:
            self.nef = nef
        else:
...
class CNN_ENCODER(nn.Module):
    ...
    def define_module(self, model):
        ...
        self.emb_features = conv1x1(768, self.nef)
        self.emb_cnn_code = nn.Linear(2048, self.nef)
        print(self.emb_features)
        print(self.emb_cnn_code)
    ...
    def init_trainable_weights(self):
        ...
        self.emb_features.weight.data.uniform_(-initrange, initrange)
        self.emb_cnn_code.weight.data.uniform_(-initrange, initrange)
        print(self.emb_features.weight.data.uniform_(-initrange, initrange).size())
        print(self.emb_cnn_code.weight.data.uniform_(-initrange, initrange).size())
...
#     def __init__(self, emb_size, hidden_size, vocab_size, nlayers=1, bidirectional=True, rec_unit='LSTM', dropout=0.5, model='../models/pytorch/'):
#         """
#         Based on https://github.com/komiya-m/MirrorGAN/blob/master/model.py
#         :param emb_size: size of word embeddings
#         :param hidden_size: size of hidden state of the recurrent unit
...
#         :param rec_unit: type of recurrent unit (default=gru)
#         """
#         print(emb_size, hidden_size, vocab_size)
#         self.model = model
#         self.dropout = dropout
#         self.nlayers = nlayers
#         self.bidirectional = bidirectional
